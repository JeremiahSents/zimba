...
type Allocation = { id: number; name: string; budget: string; unitCost: string; spent: string }
const initialRows: Allocation[] = [
  { id: 1, name: "Land", budget: "", unitCost: "", spent: "" },
  { id: 2, name: "Labour", budget: "", unitCost: "", spent: "" },
  { id: 3, name: "Materials", budget: "", unitCost: "", spent: "" },
]
...
export function ProjectCreatePage() {
  ...
  const columns = useMemo<ColumnDef<Allocation>[]>(() => [
    ...
    ...(["budget", "unitCost", "spent"] as const).map((field) => ({ accessorKey: field, header: field === "unitCost" ? "Unit cost" : field === "spent" ? "Total spent so far" : "Budget", cell: ({ row }: { row: { original: Allocation } }) => <Input type="number" min="0" value={row.original[field]} onChange={(event) => updateRow(row.original.id, field, event.target.value)} placeholder="0" /> })),
    { id: "actions", header: "", cell: ({ row }) => <button type="button" className="text-xs text-destructive" onClick={() => setRows((current) => current.filter((item) => item.id !== row.original.id))}>Remove</button> },
  ], [updateRow])
  ...
  return <DashboardShell title="New project" subtitle=""><form onSubmit={submit} className="grid gap-6">
    ...
    <section><div className="mb-3 flex items-center justify-between gap-4"><h2 className="font-heading text-base font-semibold">Initial allocation</h2><span className="text-xs text-muted-foreground">Total budget: {totalBudget.toLocaleString()} UGX</span></div><Table><TableHeader>{table.getHeaderGroups().map((group) => <TableRow key={group.id}>{group.headers.map((header) => <TableHead key={header.id}>{flexRender(header.column.columnDef.header, header.getContext())}</TableHead>)}</TableRow>)}</TableHeader><TableBody>{table.getRowModel().rows.map((row) => <TableRow key={row.id}>{row.getVisibleCells().map((cell) => <TableCell key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>)}</TableRow>)}</TableBody></Table><Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => setRows((current) => [...current, { id: Date.now(), name: "", budget: "", unitCost: "", spent: "" }])}>+ Add item</Button></section>
  </form></DashboardShell>
}
