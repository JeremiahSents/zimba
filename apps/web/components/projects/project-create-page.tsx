...
import { Delete02Icon, PencilEdit02Icon, Tick02Icon } from "@hugeicons/core-free-icons"
...
export function ProjectCreatePage() {
  ...
  const columns = useMemo<ColumnDef<Allocation>[]>(() => [
    { accessorKey: "name", header: "Expense name", cell: ({ row }) => <EditableText editing={editingId === row.original.id} value={row.original.name} onChange={(value) => updateRow(row.original.id, "name", value)} onStart={() => setEditingId(row.original.id)} /> },
    { accessorKey: "quantity", header: () => <div className="text-right">Quantity</div>, cell: ({ row }) => <EditableNumber editing={editingId === row.original.id} value={row.original.quantity} onChange={(val) => updateRow(row.original.id, "quantity", val)} onStart={() => setEditingId(row.original.id)} /> },
    { accessorKey: "unitCost", header: () => <div className="text-right">Unit cost</div>, cell: ({ row }) => <EditableNumber editing={editingId === row.original.id} value={row.original.unitCost} onChange={(val) => updateRow(row.original.id, "unitCost", val)} onStart={() => setEditingId(row.original.id)} /> },
    ...
    { id: "actions", header: "", cell: ({ row }) => <div className="flex items-center justify-center gap-1.5"><button type="button" aria-label={editingId === row.original.id ? "Done editing" : `Edit ${row.original.name || "item"}`} className="inline-flex size-5 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" onClick={() => setEditingId((current) => (current === row.original.id ? null : row.original.id))}><HugeiconsIcon icon={editingId === row.original.id ? Tick02Icon : PencilEdit02Icon} strokeWidth={2} className="size-4" /></button><button type="button" aria-label={`Remove ${row.original.name || "item"}`} className="inline-flex size-5 items-center justify-center rounded-md text-destructive transition-colors hover:bg-destructive hover:text-white" onClick={() => setRows((current) => current.filter((item) => item.id !== row.original.id))}><HugeiconsIcon icon={Delete02Icon} strokeWidth={2} className="size-4" /></button></div> },
  ], [updateRow, editingId])
...
function EditableText({ value, onChange, editing, onStart }: { value: string; onChange: (value: string) => void; editing: boolean; onStart: () => void }) {
  if (!editing) return <button type="button" className="w-full truncate text-left font-semibold" onClick={onStart}>{value || "Add expense name"}</button>
  return <input value={value} onChange={(event) => onChange(event.target.value)} placeholder="Add expense name" className="w-full bg-transparent font-semibold outline-none placeholder:text-muted-foreground" />
}
...
function EditableNumber({ value, onChange, editing, onStart }: { value: string; onChange: (value: string) => void; editing: boolean; onStart: () => void }) {
  if (!editing) return <button type="button" className="w-full text-left tabular-nums text-muted-foreground" onClick={onStart}>{value ? Number(value).toLocaleString() : "0"}</button>
  return <input inputMode="decimal" value={formatWithCommas(value)} onChange={(event) => { const raw = event.target.value.replace(/[^0-9.]/g, ""); if (raw.split(".").length > 2) return; onChange(raw) }} placeholder="0" className="w-full bg-transparent text-left tabular-nums outline-none placeholder:text-muted-foreground" />
}
