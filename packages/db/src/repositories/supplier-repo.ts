...
export function findSupplierForOrganization(
  executor: DatabaseExecutor,
  organizationId: string,
  supplierId: string
) {
...
export function findSupplierByNameForOrganization(
  executor: DatabaseExecutor,
  organizationId: string,
  name: string
) {
  return executor
    ...
    .from(supplier)
    .where(
      and(eq(supplier.organizationId, organizationId), eq(supplier.name, name))
    )
    .limit(1)
}
...
export async function createSupplier(
  executor: DatabaseExecutor,
  data: typeof supplier.$inferInsert
) {
...
export function listSupplierCategories(
  ...
  return executor
    ...
    .where(eq(supplierCategory.organizationId, organizationId))
    .orderBy(asc(supplierCategory.name))
}

export function findSupplierCategoryBySlug(
  executor: DatabaseExecutor,
  organizationId: string,
  slug: string
) {
  return executor
    ...
    .where(
      and(
        eq(supplierCategory.organizationId, organizationId),
        eq(supplierCategory.slug, slug)
      )
    )
    .limit(1)
}
...
export async function updateSupplierForOrganization(
  executor: DatabaseExecutor,
  organizationId: string,
  supplierId: string,
