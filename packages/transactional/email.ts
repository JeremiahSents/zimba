...
interface SendMemberInviteEmailProps {
  to: string;
  invitedByName: string;
  storeName: string;
  role: string;
...
export const sendMemberInviteEmail = async ({
  ...
  console.info("[email] Sending member invite email", { to, storeName, role });
  const { MemberInviteEmail } = await import('./emails/member-invite');
  const emailHtml = await render(React.createElement(MemberInviteEmail, {
    invitedByName,
    storeName,
    role,
    inviteUrl,
  ...
  try {
    data = await resend.emails.send({
      from: 'Vendly <noreply@shopvendly.store>',
      to,
      subject: `You've been invited to join ${storeName} on Vendly`,
      html: emailHtml,
    });
  } catch (error) {
    console.error("[email] Member invite email request failed", { to, error });
    throw error;
  }

  if (data.error) {
    console.error("[email] Member invite email rejected", { to, error: data.error });
    throw new Error(`Member invite email failed: ${data.error.message || JSON.stringify(data.error)}`);
  }

  console.info("[email] Member invite email sent", { to, id: data.data?.id ?? null });
  return data;
};
